...
const ProfessionalLoginPage = () => {
  ...
  // Note: Do NOT redirect here - let handleCredentialResponse handle first-time users
  useEffect(() => {
    if (isAuthenticated && user?.role === UserRole.PROFESSIONAL) {
      // Only redirect if already logged in on page load (not during login process)
      // This prevents interfering with the new user flow to subscription page
      const professional = (user as any).professional;
      if (professional?.subscriptionId) {
  ...
  const handleCredentialResponse = useCallback(
    async (response: { credential: string }) => {
      ...
      if (professionalGoogleLogin.fulfilled.match(result)) {
        // CRITICAL: Check if new user (no subscription) and redirect to subscription page
        const { data } = result.payload as any;
        const hasSubscription = data?.professional?.subscriptionId;

        if (!hasSubscription) {
          // New user or user without subscription - redirect to subscription page
          navigate('/professional/subscription');
        } else {
          // Existing user with subscription - redirect to calendar
          navigate('/professional/calendar');
        }
      }
    },
