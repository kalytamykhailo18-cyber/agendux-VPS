...
import { cleanupExpiredHolds } from './slot-hold.service';
...
let slotHoldCleanupInterval: NodeJS.Timeout | null = null;
let isProcessing = false;
let isCleaningDeposits = false;
let isCleaningHolds = false;
...
// Requirement 5.1: Hold expires after time limit if booking not completed
// ============================================
...
export async function cleanupExpiredSlotHolds(): Promise<void> {
  ...
  isCleaningHolds = true;

  try {
    const deletedCount = await cleanupExpiredHolds();

    if (deletedCount > 0) {
      ServiceLogger.reminder(`Cleaned up ${deletedCount} expired slot holds`);
    }
  } catch (error) {
    logger.error('Error in slot hold cleanup scheduler:', error);
  } finally {
    isCleaningHolds = false;
  }
}
...
export function startReminderScheduler(): void {
  ...
  // Run slot hold cleanup every 60 seconds
  slotHoldCleanupInterval = setInterval(cleanupExpiredSlotHolds, 60 * 1000);
  ...
  ServiceLogger.reminder('Slot hold cleanup scheduler started (interval: 60 seconds)');
}
...
export function stopReminderScheduler(): void {
  ...
  if (slotHoldCleanupInterval) {
    clearInterval(slotHoldCleanupInterval);
    slotHoldCleanupInterval = null;
    ServiceLogger.reminder('Slot hold cleanup scheduler stopped');
  }
}
...
export async function shutdownWorker(): Promise<void> {
  ...
  let waitCount = 0;
  while ((isProcessing || isCleaningDeposits || isCleaningHolds) && waitCount < 10) {
